...
router.post("/blogs/addlike/:blogId", async function (req, res, next) {
  ...
  try{
    ...
    // console.log() จำนวน Like ออกมาดู
...
router.get("/blogs/:id", function (req, res, next) {
  ...
  const promise2 = pool.query("SELECT * FROM comments c  LEFT OUTER JOIN images i  ON (i.blog_id = c.id ) WHERE c.blog_id = ? " , [req.params.id])
